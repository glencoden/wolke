...
git_response=$(git ls-remote https://github.com/glencoden/looney-api.git | egrep 'main|merge')
...
if [ $git_exit_status -eq 0 ]; then
  sed -i "" -e "5s/.*/ARG COMMIT_HASH=$commit_hash/" contexts/looney-api/Dockerfile
else
